...
class ControllerPaymentPaykun extends Controller {
    ...
    public function index() {
		...
		if(file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/paykun.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/paykun.tpl';
		} else {
...
//            throw new ValidationException("Something went wrong.".$e->getMessage(), $e->getCode(), null);
    ...
    private function getCallbackUrl($isFailed = false){
        ...
        return isset($_SERVER['HTTPS'])? HTTPS_SERVER . $callback_url : HTTP_SERVER . $callback_url;
            ...
            if($response && isset($response['status']) && $response['status'] == "1" || $response['status'] == 1 ) {
                ...
                if($payment_status === "Success") {
                    ...
                    if(($orderInfo['total']	== $resAmout)) {
                        ...
                        $this->model_checkout_order->confirm($order_id, $this->config->get('paykun_order_success_status_id'), $this->getSuccessOrderStatusHtml($paymentId, true));
                        ...
                        $this->model_checkout_order->confirm($order_id, $this->config->get('paykun_order_failed_status_id'), $this->getSuccessOrderStatusHtml($paymentId));
                    ...
                    // echo  "<pre>"; print_r($this->model_checkout_order); exit;
...
//                $this->response->setOutput($this->load->view($this->template, $data));
    ...
    public function callbackFailed() {
        ...
        try {
            ...
            if(isset($response['status']) && $response['status'] == "1" || $response['status'] == 1 ) {
                ...
                $this->model_checkout_order->confirm($order_id, $this->config->get('paykun_order_failed_status_id'), 
                    $this->getSuccessOrderStatusHtml($paymentId));
    ...
    private function _getcurlInfo($iTransactionId) {
        ...
        try {
            ...
            //echo $this->_merchantId." == ".$this->_accessToken;
